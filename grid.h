...
typedef wchar_t grid_value;

class grid_rendering_buffer {
public:
  ~grid_rendering_buffer() { delete[] m_rows; }

  //------------------------------------------------------------------------
  grid_rendering_buffer(grid_value *buf, unsigned width, unsigned height,
                        int stride)
      : m_buf(0), m_rows(0), m_width(0), m_height(0), m_stride(0),
        m_max_height(0) {
    attach(buf, width, height, stride);
  }

  //------------------------------------------------------------------------
  void attach(grid_value *buf, unsigned width, unsigned height, int stride) {
    m_buf = buf;
    m_width = width;
    m_height = height;
    m_stride = stride;
...
public:
  ...
  grid_rasterizer() : m_filling_rule(fill_non_zero) {}
  ...
  void render(Renderer &r, grid_value c, int dx = 0, int dy = 0) {
    ...
    for (;;) {
      ...
      if (cur_cell->x > x) {
        ...
        if (alpha) {
          ...
          m_scanline.add_span(x, y, cur_cell->x - x, 0);
        }
      }
    }
...
struct span_grid {
  ...
  //--------------------------------------------------------------------
  static void render(grid_value *ptr, int x, unsigned count,
                     const unsigned char *covers, grid_value c) {
    grid_value *p = ptr + x;
    do {
      *p++ = c;
    } while (--count);
  ...
  //--------------------------------------------------------------------
  static void hline(grid_value *ptr, int x, unsigned count, grid_value c) {
    grid_value *p = ptr + x;
    do {
      *p++ = c;
    } while (--count);
  ...
  //--------------------------------------------------------------------
  static grid_value get(grid_value *ptr, int x) { return ptr[x]; }
};
} // namespace agg
